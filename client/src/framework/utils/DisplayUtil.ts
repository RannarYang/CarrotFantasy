module Utils {
	...
	export class DisplayUtil {
		...
		*/
		...
		/**
		 * 显示动画特效
		 * @param effectName 动画资源名称
		 * @param container 容器
		 * @param xPos{number}
		 * @param yPos {number}
		 * @param playTimes {number} 播放次数。 参数为整数，可选参数，>=1：设定播放次数，<0：循环播放，默认值 0：不改变播放次数，
		 * @param onComplete 动画播放结束回调
		 * @param thisObj
		 * 
		 */
		public static showMovie(effectName:string,container:egret.DisplayObjectContainer,xPos:number=0,yPos:number=0,playTimes:number=-1,onComplete:Function=null,thisObj:any=null):MovieClip{
			var aniImg: MovieClip = ObjectPool.getInstance().spawn(MovieClip);
			var _onComplete:Function = onComplete;
			var _thisObj:any = thisObj;
			aniImg.loadFile(effectName,playTimes,()=>{
				this.removeFromParent(aniImg);
				if(_onComplete){
					_onComplete.call(_thisObj);
				}
			});
			aniImg.x = xPos;
			aniImg.y = yPos;
			container.addChild(aniImg);
			return aniImg;
		}
