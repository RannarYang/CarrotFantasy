class PopUpManager {
	...
	// 窗体控制 ===============================
	private _dicShowView: { [key: string]: View; } = {} // 类名 => 窗体对象
	private _showStack: {view: View, lucencyType: PopUpLucencyType}[] = [];
	
	// 弹出窗体 ==============
	public addPopUp<T extends View>(t: {new(): T}, showMode: PopUpShowMode = PopUpShowMode.Normal, lucencyType: PopUpLucencyType = PopUpLucencyType.Impenetrable, isCenter = true) : T{
		// 如果原来已有弹窗，则先删除
		let lastView: View = this.getPopUp(t);
		if(lastView) {
	...
	// 关闭弹出窗体 =============
	public removePopUp<T extends View>(t: {new(a): T}) {
		let desView: View = this.getPopUp(t);
		if(!desView) {
	...
	public getPopUp<T extends View>(t: {new(a): T}) {
